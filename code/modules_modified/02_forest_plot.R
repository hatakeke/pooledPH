# ==============================================================================
#  02_forest_plot.R - Forest Plot（森林プロット）
# ==============================================================================
#  概要: 実験ごとのランダム効果を可視化する森林プロット
#  依存: 00_setup.R, 01_main_model.R
#  出力: ForestPlot.png, ForestPlot_data.csv
# ==============================================================================
...
if (exists("model_fullExperimentalParameters_dem")) {
    ...
    forest_result <- create_forest_plot(
        ...
        colors = colors_paper,
        output_file = file.path(output_dir, "ForestPlot")
    )
    forest_plot <- forest_result$plot
    
    cat("====== Forest Plot completed ======\n")
} else {
    cat("Warning: Main model not found. Please run 01_main_model.R first.\n")
}
