...
#  出力: Caterpillar.png, Rhat_diagnostics.csv, ESS_diagnostics.csv
# ==============================================================================
...
if (exists("model_fullExperimentalParameters_dem")) {
    ...
    caterpillar <- create_caterpillar_plot(
        ...
        params = c(4, 5, 6),
        output_file = file.path(output_dir, "Caterpillar")
    )
    
    # R-hatチェック
    cat("\n--- R-hat Diagnostics ---\n")
    rhat_results <- check_rhat(model_fullExperimentalParameters_dem, output_file = file.path(output_dir, "Rhat_diagnostics"))
    
    # 有効サンプルサイズチェック
    cat("\n--- Effective Sample Size Diagnostics ---\n")
    ess_results <- check_effective_sample_size(model_fullExperimentalParameters_dem, output_file = file.path(output_dir, "ESS_diagnostics"))
