...
#  出力: ROPE_analysis.csv, ROPE_summary.csv
# ==============================================================================
...
if (exists("model_fullExperimentalParameters_dem")) {
    cat("\n====== Running ROPE Analysis ======\n")
    
    # メインモデルのROPE分析
    cat("\n--- Main Model ROPE Analysis ---\n")
    rope_main <- run_rope_analysis(model_fullExperimentalParameters_dem)
    print(rope_main)
    
    # サマリー（CSV保存）
    rope_summary <- summarize_rope(rope_main, output_file = file.path(output_dir, "ROPE_summary"))
    cat("\n--- ROPE Summary with Interpretation ---\n")
    print(rope_summary[, c("Parameter", "ROPE_Percentage", "interpretation")])
    
    # 追加モデルが存在する場合
    if (exists("model_fullExperimentalParameters_dem_order")) {
        cat("\n--- Order Model ROPE Analysis ---\n")
        rope_order <- run_rope_analysis(model_fullExperimentalParameters_dem_order)
        print(rope_order)
        summarize_rope(rope_order, output_file = file.path(output_dir, "ROPE_summary_order"))
    }
    
    if (exists("model_fullExperimentalParameters_dem_EHI")) {
        cat("\n--- EHI Model ROPE Analysis ---\n")
        rope_ehi <- run_rope_analysis(model_fullExperimentalParameters_dem_EHI)
        print(rope_ehi)
        summarize_rope(rope_ehi, output_file = file.path(output_dir, "ROPE_summary_EHI"))
    }
    
    if (exists("model_fullExperimentalParameters_dem_PDI")) {
        cat("\n--- PDI Model ROPE Analysis ---\n")
        rope_pdi <- run_rope_analysis(model_fullExperimentalParameters_dem_PDI)
        print(rope_pdi)
        summarize_rope(rope_pdi, output_file = file.path(output_dir, "ROPE_summary_PDI"))
    }
