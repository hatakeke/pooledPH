...
load_and_preprocess_data <- function(filename) {
    # データ読み込み
    data_orig <- read_excel(filename, sheet = "Modified Data")
    
    # --- データクリーニング: 年齢欠損の除外（約4名） ---
    data_orig <- filter(data_orig, !is.na(Age))
    ...
    # 被験者数
    noSubjects <- length(unique(data_orig$Subject_No))
    ...
    data_orig$Device <- factor(data_orig$Device, labels = c("Default", "for MRI", "for Hand", "Wearable"))
    ...
    # ==========================================================================
    #  基準値（Reference Level）の明示的設定
    # ==========================================================================
    data_orig$Condition <- relevel(data_orig$Condition, ref = "Sync")
    data_orig$Device <- relevel(data_orig$Device, ref = "Wearable")
    data_orig$Gender_IsMale <- relevel(data_orig$Gender_IsMale, ref = "Male")
    data_orig$Previous_Exposure <- relevel(data_orig$Previous_Exposure, ref = "Robot manipulation")
    data_orig$Cognitive_Load <- relevel(data_orig$Cognitive_Load, ref = "No")
    data_orig$Force_field <- relevel(data_orig$Force_field, ref = "No")

    # Deviceが "Wearable" のデータを基準として抽出
    ref_data <- subset(data_orig, Device == "Wearable")

    ref_mean_age <- mean(ref_data$Age)
    ref_mean_dur <- mean(ref_data$Duration_sec)

    data_orig$Age <- (data_orig$Age - ref_mean_age) / sd(data_orig$Age)
    data_orig$Duration_sec <- (data_orig$Duration_sec - ref_mean_dur) / sd(data_orig$Duration_sec)
    
    # 質問項目をordinal factorに変換（全ての質問で0-6の水準を固定）
    for (i in 1:18) {
        col_name <- paste0("Question_ID_", i)
        if (col_name %in% colnames(data_orig)) {
            data_orig[[col_name]] <- factor(round(data_orig[[col_name]], digits = 0), levels = 0:6, ordered = TRUE)
        }
    }
    for (i in 20:21) {
        col_name <- paste0("Question_ID_", i)
...
# Device の確認
if ("Device" %in% colnames(data_orig)) {
    cat("\nDevice Distribution:\n")
    print(table(data_orig$Device))
}
